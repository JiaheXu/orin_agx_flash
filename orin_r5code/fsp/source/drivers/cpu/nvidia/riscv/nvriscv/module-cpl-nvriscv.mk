...
ifeq ($(VERBOSE), 1)
$(info Including:  module-cpl-nvriscv.mk)
endif
...
MODULE_CPL_NVRISCV_NAME         := CPL_NVRISCV
...
#   is called.
CONFIG_CPU_DEPENDS              := CPL_NVRISCV CPL
$(foreach _,$(CONFIG_CPU_DEPENDS),$(eval $(call CHECK_MAKEFILE_DEFINED,$_)))

#
# C Source
MODULE_CPL_NVRISCV_C_SRC         =
MODULE_CPL_NVRISCV_C_SRC        += $(CPL_ISA_DIR)/riscv-mpu.c
ifeq ($(NVRISCV_FEATURE_DIO),y)
MODULE_CPL_NVRISCV_C_SRC        += $(CPL_ISA_DIR)/io_dio.c
endif

ifeq ("$(NVRISCV_HAS_CSB_MMIO)","n")
ifeq ("$(NVRISCV_HAS_PRI)","y")
ifeq ("$(NVRISCV_HAS_CSB_OVER_PRI)","y")
MODULE_CPL_NVRISCV_C_SRC        += $(CPL_ISA_DIR)/io_csb_over_pri.c
endif
endif
endif

ifeq ("$(NVRISCV_HAS_SBI)","y")
MODULE_CPL_NVRISCV_C_SRC        += $(CPL_ISA_DIR)/shutdown_sbi.c
else
MODULE_CPL_NVRISCV_C_SRC        += $(CPL_ISA_DIR)/shutdown_m.c
endif

#
# Assembly source
MODULE_CPL_NVRISCV_ASM_SRC       =
MODULE_CPL_NVRISCV_ASM_SRC      += $(CPL_ISA_DIR)/exception.S
MODULE_CPL_NVRISCV_ASM_SRC      += $(CPL_ISA_DIR)/start.S

#
# Special C Flags
MODULE_CPL_NVRISCV_C_FLAGS       =

#
# Special ASM Flags
MODULE_CPL_NVRISCV_ASM_FLAGS     =
